...
if [[ "$RACKET_VERSION" != '6.12' || "$TRAVIS_PULL_REQUEST" != 'false' || "$TRAVIS_BRANCH" != 'master' ]]; then
  exit 0;
fi
...
scribble +m --redirect-main http://docs.racket-lang.org/ --htmls --dest-name build ./scribblings/macro-exercises/index.scrbl
